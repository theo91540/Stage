public class Priority extends UniteTraitement
{
	...
	public String toXML(Systeme sys)
	{
		...
		//transitions entree
		codexml += "<transition><source ref=\""+init+"\"/><target ref=\""+(init+1)+"\"/><label kind=\"synchronisation\" x=\"50\" y=\"-20\">k_"+this.getSource()[0]+"_"+this.getName()+"?</label></transition>\n";
		codexml += "<transition><source ref=\""+init+"\"/><target ref=\""+(init+2)+"\"/><label kind=\"synchronisation\" x=\"-50\" y=\"-60\">k_"+this.getSource()[1]+"_"+this.getName()+"?</label></transition>\n";
		codexml += "<transition><source ref=\""+(init+2)+"\"/><target ref=\""+(init+3)+"\"/><label kind=\"synchronisation\" x=\"50\" y=\"-120\">k_"+this.getSource()[0]+"_"+this.getName()+"?</label><label kind=\"assignment\" x=\"200\" y=\"10\">x=0</label></transition>\n";
